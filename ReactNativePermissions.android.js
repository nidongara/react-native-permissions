...
class ReactNativePermissions {
  ...
  check = (permission: string, options?: CheckOptions): Promise<Status> => {
    ...
    return PermissionsAndroid.checkPermission(permissionTypes[permission]).then(
      isAuthorized => {
        if (isAuthorized) {
          return 'authorized'
        }
  ...
  requestPermission = (permission: string, options?: RequestOptions): Promise<Status> => {
    if (!permissionTypes[permission]) {
      const error = new Error(
        `ReactNativePermissions: ${
          permission
    ...
    return PermissionsAndroid.request(
      ...
      console.log('Result for permissions', result)
      return setDidAskOnce(permission).then(() => result)
    })
  }

  checkMultiplePermissions = (permissions: Array<string>): Promise<{ [string]: string }> =>
